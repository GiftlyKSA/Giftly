...
async def upload_image(user_id: int, username: str, image_type: ImageType, image: UploadFile) -> Dict[str, str]:
    """
...
async def upload_media(user_id: int, username: str, media_type: str, media: UploadFile) -> Dict[str, str]:
    """
    ...
    # Generate unique filename
    file_extension = media.filename.split('.')[-1] if '.' in media.filename else ('jpg' if media_type == 'image' else 'mp4')
    unique_filename = f"{uuid.uuid4()}.{file_extension}"
