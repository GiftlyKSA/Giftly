from fastapi import APIRouter, Depends, HTTPException, status, Query, Response, Form, File, UploadFile
...
async def send_message(
    ...
    if message_type not in valid_message_types:
        raise HTTPException(status_code=400, detail=f"Invalid message type. Valid types: {', '.join(valid_message_types)}")

    # Validate media file if provided
    media_url = None
    if media_file is not None:
        # Check file size limits
        if message_type == 'image' and media_file.size > 15 * 1024 * 1024:  # 15MB for images
            raise HTTPException(status_code=400, detail="Image file size exceeds 15MB limit")
        elif message_type == 'video' and media_file.size > 100 * 1024 * 1024:  # 100MB for videos
        ...
        file_ext = media_file.filename.split('.')[-1].lower() if '.' in media_file.filename else ''

        if message_type == 'image':
            if media_file.content_type not in allowed_image_types and file_ext not in allowed_image_exts:
                raise HTTPException(status_code=400, detail="Only image files are allowed (JPEG, PNG, GIF, WebP, SVG)")
        elif message_type == 'video':
            if media_file.content_type not in allowed_video_types and file_ext not in allowed_video_exts:
                raise HTTPException(status_code=400, detail="Only video files are allowed (MP4, AVI, MOV, MKV, WebM)")
    ...
    if message_type in ['image', 'video']:
        if not media_url:
            raise HTTPException(status_code=400, detail=f"Media file is required for {message_type} messages")
...
async def get_order_image(
    ...
    if not image_url:
        raise HTTPException(status_code=404, detail=f"No image found for image {image_number}")
