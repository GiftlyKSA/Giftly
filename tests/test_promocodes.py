...
async def test_apply_valid_promocode(client, promocode: Promocode):
    resp = await client.post("/promocodes/apply", json={
        "code": "SAVE10",
        "order_total": 10_000,
    })
...
async def test_apply_expired_promocode(client, db: AsyncSession):
    expired = Promocode(
        name="Expired", code="OLD50", description="",
        percentage=50, max_value=0, minimum_order_value=0,
...
async def test_apply_exhausted_usage_limit(client, db: AsyncSession):
    exhausted = Promocode(
        name="Full", code="FULL", description="",
        percentage=10, max_value=0, minimum_order_value=0,
...
async def test_list_active_promocodes_excludes_expired(client, db: AsyncSession):
    expired = Promocode(
        name="Exp", code="EXPIRED2", description="",
        percentage=5, max_value=0, minimum_order_value=0,
        usage_limit=0, usage_count=0,
